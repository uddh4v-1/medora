...
export type PasswordResetTemplateInput = {
  resetUrl: string;
  /** Shown as “expires in roughly X minutes” */
  ttlMinutes: number;
  /** Defaults to `"Medora"` */
  brandName?: string;
};
...
export function passwordResetPlainText(input: PasswordResetTemplateInput): string {
  ...
  const { resetUrl, ttlMinutes } = input;
  return [
...
/**
 * Responsive-friendly table layout; teal CTA aligns with Medora frontend brand accent.
 */
...
<title>${brand} — reset password</title>
...
<body style="margin:0;padding:0;background-color:#f6f8f6;-webkit-font-smoothing:antialiased;">
  <span style="display:none!important;color:transparent;height:0;max-height:0;opacity:0;overflow:hidden;mso-hide:all;">Reset your ${brand} password using the secure link inside.</span>
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;background-color:#ffffff;border-radius:14px;border:1px solid #e6ece7;overflow:hidden;">
          <tr>
            <td style="padding:28px 32px 10px;text-align:center;background-color:#2d5843;background-image:linear-gradient(135deg,#274d3b 0%,#2d5843 55%,#3a6c52 100%);">
              ...
              <h1 style="margin:14px 0 0;font-size:23px;line-height:1.25;color:#ffffff;font-weight:600;letter-spacing:-0.03em;">Reset your password</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:26px 32px 8px;color:#1f2f28;font-size:16px;line-height:1.6;">
              ...
              <p style="margin:0 0 26px;color:#5e7468;font-size:15px;line-height:1.55;">
                Use the button below to choose a new password for your ${brand} account.
              ...
              <p style="margin:0;color:#809388;font-size:13px;line-height:1.55;">
                Didn&apos;t request this email? You can safely ignore it. Your password will remain unchanged.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:14px 32px 28px;text-align:center;background-color:#fbfcfb;border-top:1px solid #eef3ef;">
              <p style="margin:0;font-size:12px;color:#8a9c92;">
                Sent by <strong style="font-weight:600;color:#5d7267;">${brand}</strong> · Secure sign-in helper
              </p>
            </td>
          </tr>
        </table>
